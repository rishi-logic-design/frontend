...
import "./invoiceDetailsReport.scss";
...
const InvoiceDetailsReport = () => {
  ...
  const fetchReport = useCallback(async () => {
    try {
      setLoading(true);
      const res = await reportService.getInvoiceDetailsReport({
        fromDate,
        toDate,
        search,
        page,
      ...
      toast.error(err?.message || "Failed to fetch Invoice Details Report");
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `invoice-details-report-${fromDate}-to-${toDate}.csv`;
    a.click();
  ...
  return (
    <div className="idr-page">
      {/* ── Header ─────────────────────────────────────────── */}
      <header className="idr-header">
        <div className="idr-header__left">
          <button
            className="back-btn"
        ...
        <div className="idr-header__right">
                ...
                <tbody>
                  {data.rows.length === 0 ? (
                    ...
                    data.rows.map((row, idx) => (
                      <tr key={idx}>
                        <td className="bill-no">{row.billNumber}</td>
                        <td className="tc date-cell">{row.billDate}</td>
