...
import "./partyWisePurchaseReport.scss";

const PAGE_SIZES = [10, 25, 50, 100];

const PartyWisePurchaseReport = () => {
  ...
  const fetchReport = useCallback(async () => {
    try {
      setLoading(true);
      const res = await reportService.getPartyWisePurchaseReport({
        fromDate,
        toDate,
        search,
        page,
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = [
      headers.join(","),
      ...data.rows.map((r) =>
        [r.serialNo, `"${r.sellerName}"`, r.totalAmount, r.purchaseCount].join(
          ",",
        ),
      ),
    ...
    a.href = url;
    a.download = `party-wise-purchase-report-${fromDate}-to-${toDate}.csv`;
    a.click();
  ...
  return (
    <div className="pwpr2-page">
      {/* ── Header ─────────────────────────────────────────── */}
      <header className="pwpr2-header">
        <div className="pwpr2-header__left">
          <button
            className="back-btn"
        ...
        <div className="pwpr2-header__right">
      ...
      {/* ── Main ───────────────────────────────────────────── */}
      <main className="pwpr2-main">
        <div className="pwpr2-card">
          {/* Controls */}
          <div className="pwpr2-controls">
          ...
          {/* Table */}
          <div className="pwpr2-table-wrap">
            {loading ? (
              <div className="pwpr2-loading">
                <div className="spinner" />
                <span>Fetching report data…</span>
              </div>
            ) : (
              <table className="pwpr2-table">
                <thead>
                  <tr>
                    <th>Serial No.</th>
                    <th>Seller Name</th>
          ...
          {/* Pagination */}
          <div className="pwpr2-pagination">
...
export default PartyWisePurchaseReport;
