...
import "./productWisePurchaseReport.scss";
...
const ProductWisePurchaseReport = () => {
  ...
  const fetchReport = useCallback(async () => {
    try {
      setLoading(true);
      const res = await reportService.getProductWisePurchaseReport({
        fromDate,
        toDate,
        search,
        page,
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = [
      headers.join(","),
      ...data.rows.map((r) =>
        [r.serialNo, `"${r.productName}"`, r.totalQty, r.totalAmount].join(","),
      ),
      ["", "Grand Total", data.grandTotalQty, data.grandTotalAmount].join(","),
    ];
    const blob = new Blob([csvRows.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `product-wise-purchase-report-${fromDate}-to-${toDate}.csv`;
    a.click();
  ...
  return (
    <div className="pwpr-page">
      {/* ── Header ─────────────────────────────────────────── */}
      <header className="pwpr-header">
        <div className="pwpr-header__left">
          <button
            className="back-btn"
        ...
        <div className="pwpr-header__right">
      ...
      {/* ── Main Card ──────────────────────────────────────── */}
      <main className="pwpr-main">
        <div className="pwpr-card">
          {/* Controls bar */}
          <div className="pwpr-controls">
          ...
          {/* Table */}
          <div className="pwpr-table-wrap">
            {loading ? (
              <div className="pwpr-loading">
                <div className="spinner" />
                <span>Fetching report data…</span>
              </div>
            ) : (
              <table className="pwpr-table">
                <thead>
                  <tr>
                    <th>Serial No.</th>
                    <th>Product Name</th>
